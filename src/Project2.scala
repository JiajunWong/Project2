...
abstract class WorkerActor extends Actor {
  ...
  def removeNeighbor(a: WorkerActor) = (neighbors = neighbors.remove(neighbors(a.id)))
...
class GossipNode extends WorkerActor {
  ...
  def receive = {
    case (i: Int, n: List[ActorRef], b: NetworkBuilder) => {
      init(i, n, b)
      sendLoopMessage
    }
    case Rumor => {
...
class PushSumNode extends WorkerActor {
  ...
  override def init(i: Int, n: List[ActorRef], b: NetworkBuilder) =
  ...
  def receive = {
    case (i: Int, n: List[ActorRef], b: NetworkBuilder) => init(i, n, b)
    case Rumor => spreadMessage()
    case PartialSum(ss, ww) =>
      count += 1
...
abstract class NetworkBuilder(val numNodes: Int, val algorithm: () => ActorRef) extends Actor {
  private var count = 0
